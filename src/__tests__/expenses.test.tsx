...
function seedLocalStorage(
  ...
  localStorage.setItem(
    ...
    JSON.stringify([{ amount: 200, startDate: sixtyDaysAgo }])
  );
}
...
describe("Expenses view", () => {
  ...
  describe("adding expenses", () => {
    it("adds a new expense via the form", async () => {
      ...
      const formCategorySelect = screen
    ...
    it("rejects a date before the first use date", async () => {
      ...
      expect(
        await screen.findByText(/date cannot be before budget start date/i)
      ).toBeInTheDocument();
    });
  });
  ...
  describe("editing expenses", () => {
    it("populates the form when editing and updates the expense", async () => {
      const user = userEvent.setup();
      const expenses = [
        makeExpense({ description: "Old lunch", amount: 12, category: "Food" }),
      ];
      renderExpensesView(expenses);
  ...
  describe("deleting expenses", () => {
    it("shows confirmation buttons when delete is clicked", async () => {
      ...
      expect(
        within(table).getByRole("button", { name: /^cancel$/i })
      ).toBeInTheDocument();
    });
    ...
    it("cancels deletion when cancel is clicked", async () => {
      ...
      expect(
        within(table).getByRole("button", { name: /^edit$/i })
      ).toBeInTheDocument();
    });
  ...
  describe("filtering expenses", () => {
    ...
    it("filters by search query", async () => {
      ...
      await waitFor(() => {
        expect(
          within(table).queryByText("Evening dinner")
    ...
    it("restores the default 30-day date range on reset", async () => {
      ...
      await waitFor(() => {
        expect(
          within(table).queryByText("Old expense")
        ).not.toBeInTheDocument();
      });
      expect(within(table).getByText("Recent")).toBeInTheDocument();
    });
...
function SettingsAndExpensesView() {
  const [activeTab, setActiveTab] = useState<"expenses" | "settings">(
    "settings"
  );
  const [editingExpense, setEditingExpense] = useState<Expense | null>(null);
  const [toast, setToast] = useState<{
...
describe("Expenses with deleted category", () => {
  it("still displays an expense whose category was deleted", async () => {
    ...
    const expenses = [
      makeExpense({ description: "Guitar strings", category: "Hobbies" }),
    ];
