...
function ContextHarness() {
  const { state, addExpense, updateExpense, deleteExpense, setWeeklyBudget, addCategory, deleteCategory, isLoaded } = useBudget();
  ...
  return (
    <div>
      ...
      <button
        onClick={() =>
          addExpense({ amount: 25, category: 'Food', description: 'Test expense', date: '2026-03-15' })
        }
      >
        Add Expense
      </button>
      <button
        onClick={() => {
          ...
          if (expense) updateExpense({ ...expense, description: 'Updated description', amount: 50 });
        }}
      >
...
describe('BudgetContext', () => {
  describe('loading from localStorage', () => {
    it('loads seeded expenses on mount', async () => {
      const expenses = [
        { id: 'e1', amount: 10, category: 'Food', description: 'Coffee', date: '2026-03-20', createdAt: '2026-03-20T00:00:00Z' },
      ];
    ...
    it('loads seeded weekly budget on mount', async () => {
      ...
      expect(await screen.findByTestId('weekly-budget')).toHaveTextContent('350');
    });

    it('loads seeded categories on mount', async () => {
      renderHarness({ [STORAGE_KEYS.CATEGORIES]: ['Rent', 'Groceries'] });

      expect(await screen.findByTestId('categories')).toHaveTextContent('Rent,Groceries');
    });
    ...
    it('uses defaults when localStorage is empty', async () => {
      ...
      expect(screen.getByTestId('categories')).toHaveTextContent(DEFAULT_CATEGORIES.join(','));
    });
  });
  ...
  describe('persisting to localStorage', () => {
    it('persists a new expense', async () => {
      ...
      const stored = getStoredValue<Array<{ description: string }>>(STORAGE_KEYS.EXPENSES);
    ...
    it('persists an updated expense', async () => {
      const expenses = [
        { id: 'e1', amount: 10, category: 'Food', description: 'Original', date: '2026-03-20', createdAt: '2026-03-20T00:00:00Z' },
      ];
      renderHarness({ [STORAGE_KEYS.EXPENSES]: expenses });
      await screen.findByTestId('expense-count');
      ...
      const stored = getStoredValue<Array<{ description: string; amount: number }>>(STORAGE_KEYS.EXPENSES);
    ...
    it('persists expense deletion', async () => {
      const expenses = [
        { id: 'e1', amount: 10, category: 'Food', description: 'To delete', date: '2026-03-20', createdAt: '2026-03-20T00:00:00Z' },
      ];
      renderHarness({ [STORAGE_KEYS.EXPENSES]: expenses });
      expect(await screen.findByTestId('expense-count')).toHaveTextContent('1');
  ...
  describe('data roundtrip', () => {
    it('data survives a remount', async () => {
      ...
      expect(await screen.findByTestId('expense-count')).toHaveTextContent('1');
      expect(screen.getByTestId('expenses-json')).toHaveTextContent('Test expense');
    });
  });
});
