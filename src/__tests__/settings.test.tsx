...
describe("Settings", () => {
  describe("weekly budget", () => {
    ...
    it("updates the budget when a valid amount is submitted", async () => {
      ...
      expect(
        JSON.parse(localStorage.getItem(STORAGE_KEYS.WEEKLY_BUDGET)!)
      ).toBe(500);
    });
    ...
    it("shows an error for zero budget", async () => {
      ...
      expect(
        screen.getByText(/budget must be greater than zero/i)
      ).toBeInTheDocument();
    });
  ...
  describe("currency", () => {
    it("displays the current currency", async () => {
      renderSettings();
      expect(
        await screen.findByDisplayValue("US Dollar ($)")
      ).toBeInTheDocument();
    });
    ...
    it("changes the currency", async () => {
      ...
      await waitFor(() => {
        expect(JSON.parse(localStorage.getItem(STORAGE_KEYS.CURRENCY)!)).toBe(
          "EUR"
        );
      });
    });
  ...
  describe("categories", () => {
    ...
    it("does not show delete buttons on default categories", async () => {
      ...
      const deleteButtons = within(section as HTMLElement).queryAllByTitle(
        "Delete"
      );
