...
jest.mock("../components/SpendingChart", () => {
  return {
    __esModule: true,
    default: function MockSpendingChart({
      data,
    }: {
      data: Array<{ week: string; spent: number; budget: number }>;
    }) {
      return (
...
function seedAndRender({
  ...
  const effectiveBudgetHistory = budgetHistory ?? [
    { amount: weeklyBudget, startDate: firstUseDate },
  ];

  localStorage.setItem(STORAGE_KEYS.EXPENSES, JSON.stringify(expenses));
  localStorage.setItem(
    STORAGE_KEYS.WEEKLY_BUDGET,
    JSON.stringify(weeklyBudget)
  );
  localStorage.setItem(
  ...
  localStorage.setItem(
    ...
    JSON.stringify(effectiveBudgetHistory)
  );
...
function getCardByTitle(title: string): HTMLElement {
  ...
  const titleEl =
    matches.find((el) => el.classList.contains("tracking-wider")) ?? matches[0];
...
function getSectionByHeading(heading: string): HTMLElement {
  ...
  const el =
    matches.find((e) => !e.classList.contains("tracking-wider")) ?? matches[0];
...
describe("Dashboard", () => {
  ...
  describe("weekly summary cards", () => {
    ...
    it("shows over budget when spending exceeds budget", async () => {
      ...
      expect(
        await screen.findByText("Over Budget This Week")
      ).toBeInTheDocument();
  ...
  describe("total saved/overspent all time", () => {
    it("calculates correctly with a single budget over multiple weeks", async () => {
      ...
      expect(
        await screen.findByText("Total Saved (All Time)")
      ).toBeInTheDocument();
    ...
    it("shows overspent when total spending exceeds total budget", async () => {
      ...
      expect(
        await screen.findByText("Total Overspent (All Time)")
      ).toBeInTheDocument();
