...
export default function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const weeklyChartData = weekRanges.slice(-8).map((range) => {
      const weekExpenseTotal = expensesInRange(range.start, range.end).reduce(
        (sum, e) => sum + e.amount,
        0
      );
  ...
  const budgetPercentage = Math.min(
    100,
    (stats.spentThisWeek / stats.weeklyBudget) * 100
  );
      ...
      <div className="bg-slate-800/80 backdrop-blur-sm rounded-2xl border border-slate-600/20 p-5">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-sm text-white font-semibold">
            ...
            <span className="text-slate-500">/ {fc(stats.weeklyBudget)}</span>
          </span>
        </div>
          ...
          <div className="bg-slate-800/80 backdrop-blur-sm rounded-2xl border border-slate-600/20 p-5">
            ...
            <div className="space-y-4">
              {stats.topCategories.map(([category, amount]) => {
                const percentage =
                  stats.totalSpent > 0 ? (amount / stats.totalSpent) * 100 : 0;
          ...
          <div className="bg-slate-800/80 backdrop-blur-sm rounded-2xl border border-slate-600/20 p-5 lg:col-span-2">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
              {Object.entries(stats.monthlyCategoryTotals)
                .sort(
                  ([, a], [, b]) =>
                    a.current + a.future - (b.current + b.future)
                )
                .reverse()
...
function StyledAmount({
  ...
  return (
    <span className="inline-flex items-baseline">
      {prefix && (
        <span className={`text-2xl font-bold ${colorClass}`}>{prefix}</span>
      )}
      <span className={`text-2xl font-bold ${colorClass}`}>{whole}</span>
      {decimal && (
        <span className={`text-base font-semibold ${centsColorClass}`}>
          {decimal}
        </span>
      )}
      {suffix && (
        <span className={`text-2xl font-bold ${colorClass}`}>{suffix}</span>
      )}
    </span>
  );
}
...
function SummaryCard({
  ...
  return (
    <div
      className={`bg-slate-800/80 backdrop-blur-sm rounded-2xl border ${colorStyles[color]} p-5 transition-colors`}
    >
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-2">
            <StyledAmount
              value={value}
              colorClass={valueColors[color]}
              centsColorClass={centsColors[color]}
            />
          </p>
        ...
        <div
          className={`w-10 h-10 rounded-xl flex items-center justify-center ${iconBgColors[color]}`}
        >
          {icon}
        </div>
      </div>
