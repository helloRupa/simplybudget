...
export default function ExpenseList({ onEdit, onToast }: ExpenseListProps) {
  ...
  const SortIndicator = ({ field }: { field: SortState['field'] }) => {
    if (sort.field !== field) return <span className="text-purple-500 ml-1">↕</span>;
    return <span className="text-purple-300 ml-1">{sort.direction === 'desc' ? '↓' : '↑'}</span>;
  };
    ...
    <div className="space-y-4">
      ...
      {filteredExpenses.length === 0 ? (
        <div className="bg-purple-800/20 rounded-2xl border border-purple-600/20 p-12 text-center">
          ...
          <p className="text-purple-300">{t('noExpenses')}</p>
        </div>
      ) : (
        <>
          {/* Desktop table */}
          <div className="hidden md:block bg-purple-800/30 backdrop-blur-sm rounded-2xl border border-purple-600/20 overflow-hidden">
            <table className="w-full">
              <thead>
                <tr className="border-b border-purple-600/20">
                  <th
                    className="text-left text-xs font-semibold text-purple-300 uppercase tracking-wider px-5 py-3 cursor-pointer hover:text-purple-100"
                  ...
                  <th className="text-left text-xs font-semibold text-purple-300 uppercase tracking-wider px-5 py-3">
                    {t('description')}
                  </th>
                  <th
                    className="text-right text-xs font-semibold text-purple-300 uppercase tracking-wider px-5 py-3 cursor-pointer hover:text-purple-100"
                    onClick={() => handleSort('amount')}
                  >
                    {t('amount')} <SortIndicator field="amount" />
                  </th>
                  <th className="text-right text-xs font-semibold text-purple-300 uppercase tracking-wider px-5 py-3">
                    {t('actions')}
                  </th>
                </tr>
              </thead>
              <tbody>
                {filteredExpenses.map((expense) => (
                  <tr
                    key={expense.id}
                    className="border-b border-purple-600/10 hover:bg-purple-700/20 transition-colors"
                  >
                    <td className="px-5 py-3 text-sm text-purple-100">{formatDate(expense.date)}</td>
                    ...
                    <td className="px-5 py-3 text-sm text-purple-200">
                      {expense.description || <span className="text-purple-500 italic">—</span>}
                    </td>
                      ...
                      {deletingId === expense.id ? (
                        <div className="flex gap-2 justify-end">
                          ...
                          <button
                            ...
                            className="text-xs px-2.5 py-1 rounded-lg bg-purple-700/30 text-purple-300 hover:bg-purple-700/50 transition-colors"
                          >
                            {t('cancel')}
                          </button>
                        </div>
                        ...
                        <div className="flex gap-2 justify-end">
                          <button
                            ...
                            className="text-xs px-2.5 py-1 rounded-lg bg-purple-600/20 text-purple-300 hover:bg-purple-600/30 hover:text-white transition-colors"
                          >
                            {t('edit')}
                          </button>
                          <button
          ...
          <div className="md:hidden space-y-3">
            {filteredExpenses.map((expense) => (
              <div
                key={expense.id}
                className="bg-purple-800/30 backdrop-blur-sm rounded-xl border border-purple-600/20 p-4"
              >
                <div className="flex items-start justify-between mb-2">
                  <div>
                    ...
                    <p className="text-xs text-purple-400 mt-1">{formatDate(expense.date)}</p>
                  </div>
                  <p className="text-lg font-bold text-white">{formatCurrency(expense.amount)}</p>
                </div>
                {expense.description && (
                  <p className="text-sm text-purple-200 mb-3">{expense.description}</p>
                )}
                <div className="flex gap-2 justify-end">
                  {deletingId === expense.id ? (
                    <>
                      ...
                      <button onClick={() => setDeletingId(null)} className="text-xs px-3 py-1.5 rounded-lg bg-purple-700/30 text-purple-300">
                        {t('cancel')}
                      </button>
                    </>
                  ) : (
                    <>
                      <button onClick={() => onEdit(expense)} className="text-xs px-3 py-1.5 rounded-lg bg-purple-600/20 text-purple-300">
                        {t('edit')}
                      </button>
