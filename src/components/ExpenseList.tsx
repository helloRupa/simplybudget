...
function RecurringIcon() {
  return (
    <svg
      className="w-3.5 h-3.5 text-teal-400/60 shrink-0"
      fill="none"
      viewBox="0 0 24 24"
      stroke="currentColor"
      strokeWidth={2}
    ...
    return (
      <div className="flex gap-2 justify-end">
        <button
          ...
          className={`text-xs ${padding} rounded-lg bg-red-500/20 text-red-300 hover:bg-red-500/30 transition-colors`}
        >
          {t("delete")}
        </button>
        <button
          onClick={onCancelDelete}
          className={`text-xs ${padding} rounded-lg bg-slate-600/30 text-slate-300 hover:bg-slate-600/50 transition-colors`}
        >
          {t("cancel")}
        </button>
      </div>
  ...
  return (
    <div className="flex gap-2 justify-end">
      <button
        ...
        className={`text-xs ${padding} rounded-lg bg-teal-500/15 text-teal-300 hover:bg-teal-500/25 hover:text-white transition-colors`}
      >
        {t("edit")}
      </button>
      <button
        onClick={() => onDelete(expense.id)}
        className={`text-xs ${padding} rounded-lg bg-red-500/10 text-red-400/70 hover:bg-red-500/20 hover:text-red-300 transition-colors`}
      >
        {t("delete")}
      </button>
    </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-600/20">
                  ...
                  >
                    {t("category")}{" "}
                    <SortIndicator field="category" sort={sort} />
                  </th>
              ...
              <tbody>
                {filteredExpenses.map((expense) => (
                  ...
                  >
                    ...
                    <td
                      className={`px-5 py-3 text-sm text-right font-semibold ${expense.amount < 0 ? "text-teal-400" : "text-amber-300"}`}
                    >
                      {fc(expense.amount)}
                    </td>
            ...
            {filteredExpenses.map((expense) => (
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  ...
                  <p
                    className={`text-lg font-bold ${expense.amount < 0 ? "text-teal-400" : "text-amber-300"}`}
                  >
                    {fc(expense.amount)}
                  </p>
                </div>
